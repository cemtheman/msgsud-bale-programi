-- Management / M24.6
-- Token-Controlled Exact-Source Partial Recovery Apply
--
-- Applies exactly the conflict-free 28-card M24.5 batch.
--
-- Safety contract:
--   * SQL-Editor/postgres only
--   * exact M24.5 plan token required
--   * revision serialized FOR UPDATE
--   * all 28 targets are re-previewed and must remain conflict-free
--   * card graph is unchanged
--   * the existing 228 placements are guarded against mutation
--   * only the exact previewed cards/source slots are placed
--   * generic forced propagation is NOT invoked
--   * one non-revertible recovery history epoch + 28 AUTO PLACE children
...
create table public.management_exact_partial_recovery_runs (
  ...
  constraint management_exact_partial_recovery_runs_plan_unique
    unique (schedule_revision_id, plan_token),
  constraint management_exact_partial_recovery_runs_nonnegative
    check (
      target_count >= 0
...
alter table public.management_exact_partial_recovery_runs
  enable row level security;

revoke insert, update, delete
  on public.management_exact_partial_recovery_runs
...
create policy management_exact_partial_recovery_runs_read
  on public.management_exact_partial_recovery_runs
  for select
  to authenticated
...
-- -------------------------------------------------------------------------
-- TOKEN-CONTROLLED APPLY
-- -------------------------------------------------------------------------

create or replace function public.management_apply_exact_partial_recovery_v2(
...
declare
  ...
  v_before_partial_run_count integer;
  ...
  v_after_historical_run_count integer;
  v_after_partial_run_count integer;
...
begin
  if session_user <> 'postgres' then
    raise exception
      'M24.6 exact partial recovery apply is SQL-Editor/postgres only'
      using errcode = '42501';
  end if;
  ...
  if p_expected_plan_token is null
    ...
    raise exception
      'M24.6 apply requires an exact M24.5 plan token';
  end if;
  ...
  if not found then
    raise exception
      'M24.6 schedule revision not found: %',
  ...
  if v_revision.revision_status <> 'DRAFT'
    ...
    raise exception
      'M24.6 requires a DRAFT revision on a DRAFT requirement set';
  end if;
  ...
  if exists (
    ...
    raise exception
      'M24.6 plan token was already applied';
  end if;

  -- Re-preview after taking the revision lock. The preview refreshes the derived
  -- candidate domain, so every candidate id below belongs to this serialized
  -- current state.
  v_preview :=
  ...
  if v_preview ->> 'planToken'
    ...
    raise exception
      'M24.6 exact partial recovery preview is stale: expected %, current %',
      p_expected_plan_token,
      v_preview ->> 'planToken';
  end if;

  if not coalesce(
    (v_preview #>> '{summary,canApplyWholeBatch}')::boolean,
    false
  ) then
    raise exception
      'M24.6 current exact-source batch is not conflict-free';
  end if;

  if coalesce(
       (v_preview #>> '{summary,conflictPairCount}')::integer,
       -1
     ) <> 0
     or coalesce(
       (v_preview #>> '{summary,conflictedCardCount}')::integer,
       -1
     ) <> 0 then
    raise exception
      'M24.6 current exact-source preview contains target conflicts';
  end if;
  ...
  v_target_count :=
    coalesce(
      (v_preview #>> '{summary,targetCount}')::integer,
      -1
    );

  v_target_period_count :=
    coalesce(
      (v_preview #>> '{summary,targetPeriodCount}')::integer,
      -1
    );

  v_resolved_target_count :=
    coalesce(
      (v_preview #>> '{summary,resolvedTargetCount}')::integer,
      -1
    );

  v_provisional_target_count :=
    coalesce(
      (v_preview #>> '{summary,provisionalTargetCount}')::integer,
      -1
    );

  if v_target_count <> 28
     or jsonb_array_length(v_targets) <> 28
     or v_target_period_count <> 46
     or v_resolved_target_count <> 15
     or v_provisional_target_count <> 13 then
    raise exception
      'M24.6 preview cardinality/certainty mismatch: targets %, json targets %, periods %, resolved %, provisional %',
      v_target_count,
      jsonb_array_length(v_targets),
      v_target_period_count,
      v_resolved_target_count,
      v_provisional_target_count;
  end if;
  ...
  select count(*)
  into v_before_partial_run_count
  from public.management_exact_partial_recovery_runs run
  where run.schedule_revision_id = p_schedule_revision_id;

  if v_before_card_count <> 292
     or v_before_placement_count <> 228
     or v_before_move_count <> 343
     or v_before_historical_run_count <> 1
     or v_before_partial_run_count <> 0 then
    raise exception
      'M24.6 current state no longer matches accepted M24.5 baseline: cards %, placements %, moves %, historical runs %, partial runs %',
      v_before_card_count,
      v_before_placement_count,
      v_before_move_count,
      v_before_historical_run_count,
      v_before_partial_run_count;
  end if;
  ...
  if v_before_structure_hash is distinct from
    ...
    raise exception
      'M24.6 preview persistent-state hashes no longer match';
  end if;

  -- Guard every existing placement semantically. The exact batch must only add
  -- new placement rows.
  drop table if exists pg_temp.m246_existing_placement_guard;

  create temporary table m246_existing_placement_guard
  on commit drop
  as
  select
    placement.id,
  ...
  -- Create a non-revertible recovery epoch. Card graph does not change; the
  -- STRUCTURE action is used strictly as the existing permanent history-boundary
  -- mechanism so reconstructed baseline placements cannot be undone as ordinary
  -- user scheduling edits.
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'engine_version', 'M24.6-v1',
      'structure_kind',
        'HISTORICAL_EXACT_PARTIAL_RECOVERY_V2',
  ...
  loop
    ...
    if not exists (
      ...
      raise exception
        'M24.6 target card identity drift: %',
        v_card_id;
    end if;
    ...
    if exists (
      ...
      raise exception
        'M24.6 target card became placed: %',
        v_card_id;
    end if;
    ...
    if not found
       ...
       or to_jsonb(coalesce(v_candidate.warning_codes, array[]::text[]))
            is distinct from
          coalesce(v_target -> 'warningCodes', '[]'::jsonb) then
      raise exception
        'M24.6 candidate drift for target card %',
        v_card_id;
    end if;
    ...
    if v_placement.teacher_resolution_status is distinct from
      ...
      raise exception
        'M24.6 placement certainty drift for card %',
        v_card_id;
    end if;
  ...
  if v_inserted_count <> 28
     or v_inserted_period_count <> 46
     or v_inserted_resolved_count <> 15
     or v_inserted_provisional_count <> 13 then
    raise exception
      'M24.6 inserted batch mismatch: cards %, periods %, resolved %, provisional %',
      v_inserted_count,
      v_inserted_period_count,
      v_inserted_resolved_count,
      v_inserted_provisional_count;
  ...
  -- Existing placement rows must be unchanged.
  if exists (
    select 1
    from pg_temp.m246_existing_placement_guard before_row
    ...
    raise exception
      'M24.6 existing placement guard failed';
  end if;

  -- Refresh only after the exact batch. Deliberately do NOT invoke generic
  -- forced propagation.
  ...
  select count(*)
  into v_after_historical_run_count
  from public.management_historical_recovery_runs run
  where run.schedule_revision_id = p_schedule_revision_id;

  select count(*)
  into v_after_partial_run_count
  from public.management_exact_partial_recovery_runs run
  where run.schedule_revision_id = p_schedule_revision_id;
  ...
  if v_after_structure_hash is distinct from v_before_structure_hash
     or v_after_card_hash is distinct from v_before_card_hash then
    raise exception
      'M24.6 unexpectedly changed requirement/card structure';
  end if;

  if v_after_card_count <> 292
     or v_after_placement_count <> 256
     or v_after_move_count <> 372
     or v_after_historical_run_count <> 1
     or v_after_partial_run_count <> 0
     or v_remaining_card_count <> 36 then
    raise exception
      'M24.6 post-apply state mismatch: cards %, placements %, moves %, historical runs %, pre-audit partial runs %, remaining %',
      v_after_card_count,
      v_after_placement_count,
      v_after_move_count,
      v_after_historical_run_count,
      v_after_partial_run_count,
      v_remaining_card_count;
  end if;
  ...
  if v_public_session_count <> 517
    ...
    raise exception
      'M24.6 modified public projection: sessions %, groups %',
  ...
  if not coalesce(
    ...
    raise exception
      'M24.6 public baseline drift detected';
  end if;

  insert into public.management_exact_partial_recovery_runs (
    schedule_revision_id,
    engine_version,
    plan_token,
    root_transaction_id,
  ...
  return jsonb_build_object(
    ...
    'cardCountBefore', v_before_card_count,
    ...
    'nextStep',
      'M24_7_REMAINING_36_TRIAGE'
  );
end
$$;

revoke all
  on function public.management_apply_exact_partial_recovery_v2(
    uuid,
    text
  )
  from public, anon, authenticated;

comment on function public.management_apply_exact_partial_recovery_v2(
  uuid,
  text
) is
  'M24.6 SQL-Editor/postgres-only atomic exact-source partial recovery. Requires the exact conflict-free M24.5 plan token, places only the 28 previewed exact historical cards, preserves the existing 228 placements and complete card graph, records one non-revertible recovery history epoch with 28 AUTO PLACE children, and leaves the public projection untouched.';
...
begin
  ...
  if v_revision_id is null then
    raise exception
      'M24.6 current term-1 DRAFT not found';
  end if;
  ...
  if v_sessions <> 517
    ...
    raise exception
      'M24.6 installation must not apply partial recovery: sessions %, groups %, cards %, placements %, moves %, historical runs %, partial runs %',
      v_sessions,
      v_groups,
      v_cards,
      v_placements,
  ...
  if has_function_privilege(
    ...
    raise exception
      'M24.6 exact partial recovery apply must remain SQL-Editor only';
  end if;
  ...
  if has_function_privilege(
    ...
    raise exception
      'M24.6 must not unlock publication';
  end if;
  ...
  if has_function_privilege(
    ...
    raise exception
      'M24.6 must not unlock term template apply';
  end if;
end
$$;
