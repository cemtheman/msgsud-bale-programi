-- Management / M20.1.1
-- Hotfix: PostgreSQL has no min(uuid) aggregate.
-- Source-run teacher/room values are constant within each run by construction,
-- so take the first ordered UUID instead.
--
-- No placements, move history, publication state, or public schedule rows mutate.
...
begin
  if not public.has_management_role('VIEWER') then
    raise exception 'M20.1.1 management VIEWER role required'
      using errcode = '42501';
  end if;
  ...
  if not found then
    raise exception 'M20.1.1 schedule revision not found: %',
  ...
  if v_revision.revision_status <> 'DRAFT'
    ...
    raise exception
      'M20.1.1 recovery preview requires a DRAFT revision on a DRAFT requirement set';
  end if;
  ...
  -- later managed publication may replace the public projection. M20.1.1 is for
...
comment on function public.management_preview_existing_schedule_recovery(uuid) is
  'M20.1.1 preview-only exact historical placement recovery. UUID source resources are selected with ordered array aggregation because PostgreSQL does not define min(uuid). No apply endpoint exists.';
...
begin
  ...
  if v_sessions <> 517 or v_groups <> 609 then
    raise exception
      'M20.1.1 hotfix modified public projection: sessions %, groups %',
      v_sessions,
      v_groups;
  end if;
  ...
  if exists (
    ...
    raise exception
      'M20.1.1 expected no managed publication during recovery preview';
  end if;
end
$$;
