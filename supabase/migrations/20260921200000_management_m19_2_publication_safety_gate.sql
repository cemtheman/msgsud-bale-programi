...
-- STATE TOKEN
-- -------------------------------------------------------------------------
-- Defined after preview declaration textually, then preview is replaced once
-- more below so PostgreSQL resolves the helper at creation time.
...
-- Recreate preview now that state-token helper exists.
-- PostgreSQL resolves function references at CREATE FUNCTION time.
create or replace function public.management_preview_publication(
  p_schedule_revision_id uuid
)
returns jsonb
