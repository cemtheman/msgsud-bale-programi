...
--
-- No structural mutation command is introduced here.
-- The RPC compares a proposed weekly load / partition / term status with the
-- current DRAFT card graph and reports preservation, creation, removal,
-- placed-card impact, and ambiguity.
--
...
declare
  ...
  v_preferred smallint[] := coalesce(
    p_preferred_partition,
    array[]::smallint[]
  );
  v_allowed jsonb := coalesce(p_allowed_partitions, '[]'::jsonb);
  v_current_card_count integer := 0;
  v_current_placed_count integer := 0;
  v_proposed_card_count integer := 0;
  v_removed_placed_count integer := 0;
...
begin
  ...
  if p_term_status is null
     or p_term_status not in ('ACTIVE', 'INACTIVE', 'UNKNOWN') then
  ...
  if exists (
    ...
    raise exception 'M17.2 allowed partitions must contain arrays only';
  end if;
  ...
  if exists (
    ...
    where duration <= 0 or duration > 12
  ) then
  ...
  if p_term_status = 'ACTIVE' then
    ...
    if jsonb_array_length(v_allowed) > 0
       and not exists (
         select 1
         from jsonb_array_elements(v_allowed) alternative
         where alternative = to_jsonb(v_preferred)
       ) then
      raise exception 'M17.2 preferred partition must be included in allowed partitions';
    end if;
  end if;
  ...
  proposed_blocks as (
    ...
    from unnest(
      case
        when p_term_status = 'ACTIVE' then v_preferred
        else array[]::smallint[]
      end
  ...
  return jsonb_build_object(
    ...
    'candidateRebuildCardCount', v_proposed_card_count,
    'previewOnly', true
  );
end
$$;
