-- Management v0.1 / M9.1
-- Deterministic audit ordering fix for undo/redo.
--
-- PostgreSQL now() / default timestamptz values are transaction-stable. A
-- rollback-only smoke test correctly exposed that several USER actions executed
-- in one transaction can therefore share created_at. UUID lexical ordering is
-- not a causal order and must not decide LIFO/redo invalidation.
--
-- Add a monotonic history sequence and use it for stack ordering. No scheduling
-- state or published projection is changed by this migration.

begin;

alter table public.move_transactions
  add column history_sequence bigint generated by default as identity;

create unique index move_transactions_history_sequence_uidx
  on public.move_transactions (history_sequence);
...
comment on column public.move_transactions.history_sequence is
  'M9.1 monotonic audit order. Used for deterministic LIFO undo/redo ordering and redo-branch invalidation; created_at remains presentation/audit time.';
...
begin
  ...
  order by mt.history_sequence desc
  limit 1;
  ...
  select
    array_agg(mt.id order by mt.history_sequence),
    count(*)
  into
    v_chain_ids,
    v_chain_count
...
begin
  ...
  if v_placement_count <> 0 or v_transaction_count <> 0 then
    raise exception
      'M9.1 installation requires clean rollback state: placements %, transactions %',
      v_placement_count,
      v_transaction_count;
  end if;
  ...
  if v_published_session_count <> 517 then
    raise exception
      'M9.1 changed published schedule projection unexpectedly: %',
  ...
  if v_revision_id is null then
    raise exception 'M9.1 expected the canonical v1 DRAFT revision';
  end if;

  update public.schedule_revisions revision
  set validation_summary = coalesce(revision.validation_summary, '{}'::jsonb)
    || jsonb_build_object(
      'redo_history_ordering', 'M9.1-HISTORY-SEQUENCE',
